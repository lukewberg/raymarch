...
pub fn march(scene: &Scene, origin: &Vec3, direction: &Vec3) -> RayResult {
    ...
    let normal = calculate_surface_normal2(&scene.scene_objects[object_index], &position, last_closest);
    // let normal = ((*(scene.scene_objects[object_index].pos()) - position) * -1.0).normalize_new();
    ...
    RayResult {
        point: position,
        intensity: light_diff.clamp(0.0, 1.0),
    }
}
...
pub fn calculate_surface_normal2(
    scene_object: &Box<dyn SceneObject>,
    point: &Vec3,
    distance: f32,
) -> Vec3 {
